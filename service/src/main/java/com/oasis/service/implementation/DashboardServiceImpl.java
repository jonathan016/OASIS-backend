...
public class DashboardServiceImpl
    ...
    public List< RequestModel > getRequestsList(
        ...
        for (String supervisedEmployeeUsername : supervisedEmployeeUsernameList) {
            assignedRequests.addAll(
                    requestRepository.findAllByUsernameAndStatus(supervisedEmployeeUsername, requestStatus));

            boolean isAdminOrSuperior =
        ...
        switch (role) {
            ...
            case ServiceConstant.ROLE_SUPERIOR:
                if (tab.equals(ServiceConstant.TAB_OTHERS)) {
                    ...
                    if (tab.equals(ServiceConstant.TAB_SELF)) {
    ...
    public List< DashboardRequestUpdateResponse.RequestUpdateModel > mapRequests(
        ...
        for (RequestModel requestUpdate : requestUpdates) {
            DashboardRequestUpdateResponse.RequestUpdateModel.Request request =
                    ...
                    requestUpdate.get_id(), requestUpdate.getStatus(), requestUpdate.getRequestNote());
            DashboardRequestUpdateResponse.RequestUpdateModel.Employee employee =
                    ...
                    requestUpdate.getUsername(), getEmployeeData(requestUpdate.getUsername()).getName());
            DashboardRequestUpdateResponse.RequestUpdateModel.Supervisor supervisor =
                    ...
                    getEmployeeSupervisorData(requestUpdate.getUsername()).getSupervisorUsername(), getEmployeeData(
                    getEmployeeSupervisorData(requestUpdate.getUsername()).getSupervisorUsername()).getName());
            DashboardRequestUpdateResponse.RequestUpdateModel.Asset asset =
                    new DashboardRequestUpdateResponse.RequestUpdateModel.Asset(
                    requestUpdate.getSku(), getAssetData(requestUpdate.getSku()).getName(),
                    requestUpdate.getQuantity()
            );

            mappedRequests.add(
                    new DashboardRequestUpdateResponse.RequestUpdateModel(request, employee, supervisor, asset));
        }
    ...
    /*--------------Status Section--------------*/
    @Override
    public Map< String, Integer > getStatusSectionData(final String username)
            throws
            DataNotFoundException,
            BadRequestException {
    ...
    public List< DashboardRequestUpdateResponse.RequestUpdateModel > getRequestUpdateSectionData(
        ...
        List< RequestModel > requestUpdates = new ArrayList<>(fillData(username, tab, role));

        if (requestUpdates.size() == 0) {
            throw new DataNotFoundException(REQUESTS_NOT_FOUND);
        }

        if ((int) Math.ceil((float) requestUpdates.size() / ServiceConstant.DASHBOARD_REQUEST_UPDATE_PAGE_SIZE) <
            page) {
            throw new DataNotFoundException(REQUESTS_NOT_FOUND);
        }
